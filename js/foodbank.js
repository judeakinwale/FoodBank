$(document).ready(function () {
    ...
    var customBasePath = "Foodbank/v1.2"; //global variable
    var customBasePath = "";
 ...
 $('.slider-range-price').each(function(){
          ...
          $.ajax({
              ...
              .done(function( msg ) {
                    ...
                    if(status.status == 'ok'){
                        $('.success').empty();
                        $('.error').empty();
                         var msg = ' You have successfully subscribed to Foodbank newsletters and updates';
                          $('.success').append(msg);
                          $('.success').show();
                        getSubscriptionStatus();
  ...
  function unSubscribe(){
    ...
    .done(function( msg ) {
      ...
      if(status.status == 'ok'){  
        $('.success').empty();
        $('.error').empty(); 
         var msg = ' You have successfully unSubscribed to Foodbank newsletters and updates';
          $('.success').append(msg);
          $('.success').show();
        getSubscriptionStatus();
          return false;
  ...
  $('#expiry-datepicker').change(function(){
    if( $('#setDiscount').is(":unchecked")){
       ...
       if (marketPrice) {
          var specialPrice =getFoodbankPrice(marketPrice,discount);
          $("#FoodbankPrice").val(specialPrice);
       }
     }
  });
  ...
  $('#setDiscount').change(function(){
    if($(this).is(":checked")){
        ...
        $('#FoodbankPrice').attr("disabled",false);
      }
      else{
        $('#discount').attr("disabled",true);
         $('#FoodbankPrice').attr("disabled",true);
      }
   });
  ...
  $('#FoodbankPrice').keyup(function(){
     var FoodbankPrice =  $('#FoodbankPrice').val();
     var marketPrice = $('#productPrice').val();
     if(marketPrice == ""){
       $("#discount").val(0);
     }
     else{
       var rate = (1 - (FoodbankPrice/marketPrice)) * 100 ;
       $("#discount").val(rate);
     }
  });
...
function getFoodbankPrice(marketPrice,discount){
  if(discount == 0){
    var FoodbankPrice = marketPrice;
   }
   else{
     var discountPrice = (discount/100) * marketPrice;
     var FoodbankPrice = marketPrice - discountPrice;
   }  
   return FoodbankPrice;
}
});
//};
